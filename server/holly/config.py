...
class Config:
    ...
    # Holiday overlay: detections of these classes wear a Santa hat instead of a box (COCO 16 = dog)
    santa_hat: bool = field(default_factory=lambda: _bool(_env("SANTA_HAT", "false")))
    santa_hat_classes: List[int] = field(default_factory=lambda: list(ast.literal_eval(_env("SANTA_HAT_CLASSES", "[16]"))))
    santa_hat_scale: float = field(default_factory=lambda: float(_env("SANTA_HAT_SCALE", "0.6")))
    santa_hat_image: Path = field(
        default_factory=lambda: Path(_env("SANTA_HAT_IMAGE", str(Path(__file__).parent / "assets" / "santa_hat.png")))
    )
